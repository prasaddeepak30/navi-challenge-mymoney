...
import static com.navi.challenge.mymoney.util.MyMoneyConstants.CANNOT_REBALANCE;
...
public class MyMoneyServiceImpl implements MyMoneyService {
    ...
    private MyMoneyFundPortfolio createMyMoneyFunds(
        ...
        return new MyMoneyFundPortfolio(FundList);
    }
    ...
    private MyMoneyFundPortfolio applyMarketChange(
            MyMoneyFundPortfolio carryOverBalance, Map<Asset, Double> changeRate) {
            ...
            IntStream.range(0, funds.size())
                    .forEach(
                            index -> {
                                ...
                                Fund.setAmount(Math.floor(Fund.getAmount() + sipAmount));
                            });
        }
        return carryOverBalance;
    }
    ...
    private MyMoneyFundPortfolio doReBalance(MyMoneyFundPortfolio currentFunds) {
        ...
        funds.forEach(
                entity -> {
                    double desiredWeight = dataStub.desiredWeights.get(entity.getAsset());
